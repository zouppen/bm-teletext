...
def test_decodes_socketio_mqtt_payload_json_string() -> None:
    assert decode_mqtt_payload({"payload": '{"ContextID": 244123}'}) == {
        "ContextID": 244123
    }
...
def test_appends_all_matching_events_without_deduplication() -> None:
    ...
    message = {
        "payload": '{"ContextID": 244123, "SourceID": 9999999, "SessionID": "abc"}'
    }
    ...
    assert store.payloads == [
        {"ContextID": 244123, "SourceID": 9999999, "SessionID": "abc"},
        {"ContextID": 244123, "SourceID": 9999999, "SessionID": "abc"},
    ]


def test_skips_non_matching_context_id_even_when_source_id_matches() -> None:
    store = FakeStore()
    collector = LastHeardCollector(
        store=store,
        url="https://example.invalid",
...
def test_collector_uses_configured_context_id_filter() -> None:
    store = FakeStore()
    collector = LastHeardCollector(
        store=store,
        url="https://example.invalid",
        socketio_path="/lh",
        context_id_filter=ContextIdFilter(r"^999.*$"),
    )

    assert collector.handle_mqtt({"payload": '{"ContextID": 999123}'})
    assert not collector.handle_mqtt({"payload": '{"ContextID": 244123}'})
    assert store.payloads == [{"ContextID": 999123}]
